...
> **記事のURLは「公開日8桁 + 記事番号」です**(旧WordPressと同じ形式)。
> 例: 2026年8月23日に書いた記事番号1108の記事 → `202608231108` → `https://techlog.ta-yan.ai/202608231108/`
>
> CMSの「記事番号」欄に**前の記事の番号+1**を入れるだけで、URLは自動で組み立てられます(日付部分は自動)。
> 記事番号は旧サイトからの通し番号で、**1107まで使用済み**です(次に書く記事は 1109 から)。
> 番号は frontmatter の `wpId` に保存されます。
>
> Markdownを直接置く場合は、ファイル名がそのままURLになります(同じ形式に揃えても、英数字の任意スラッグでも構いません)。
>
> 補足: URLの日付部分は「CMSで保存した時刻」基準です。下書きを寝かせてから公開すると公開日とずれます。
> 気になる場合はCMSの3点メニュー「スラッグを編集」で変更できます(その際は旧URLからの301を `public/_redirects` に追記してください)。
...
wpId: 1109                    # 記事番号。URLの後半になる(前の記事の番号+1)
---
